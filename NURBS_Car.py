...
# ==========================================
# ⚠️ ここから下は一時的な「過去データ更新用」コードです
# ==========================================
import time

st.markdown("---")
st.markdown("### 【管理者用】過去データの一括更新ツール")
st.warning("各シートのG列（座標変化量）を自動計算します。終わったらこの部分は消して構いません。")

if st.button("既存データを一括更新する (一回だけ押してください)"):
    if client is None:
        st.error("Google Sheetsの認証ができていません。")
    else:
            ...
            try:
                ...
                for ws in worksheets:
                    ...
                    for i, row in enumerate(all_values):
                        ...
                        ctrlpts_str = row[5]   # F列: 座標
                        diff_str = row[6]      # G列: 座標変化量
                        ...
                        if diff_str.strip() == "" and model_name in CAR_MODELS_CTRLPTS:
                            try:
                                ...
                                if len(current_pts) == len(initial_pts):
                                    ...
                                    updates.append({'range': f'G{row_index}', 'values': [[new_diff_str]]})
                            except Exception:
                                pass
